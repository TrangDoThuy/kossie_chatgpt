...
messages = [
    ...
    You will talk with client  who is seeking mental help. All conversation will be in Cantonese, in casual speech style.
    ...
    If they ask what should they do or ask for any suggestion aor guidance, please say that: "我哋會為你尋找到最適合你嘅心理輔導師，提供個人化嘅指導同埋支持".
    ...
    Finish with "我哋會為你尋找到最適合你嘅心理輔導師，提供個人化嘅指導同埋支持"
    """},
    {"role": "assistant", "content": "你好,我是Kaia,一名Kossie教練AI助手。你今天感覺如何?"}
]
category_tag = {
...
def summary():
    ...
    Please do not mention any suggestion or advise from assistant.
...
def summary_cantonese():
    _check_rate_limit()
    prompt = f"""
    Your task is to generate a short summary about user mental issue from a conversation in a pre-coaching session between user and assistant.

    Given the conversation below delimited by triple backticks, please summarize the content of user in at most 250 words. 
    The summary should be written from client perspective in Cantonese, using subject as "我". 
    Remove all content of assistant in the summary.
    Please do not mention any suggestion or advise from assistant.
    End with 3 questions that you think user would like to ask the coach to solve his/her problem. 
    ...
    global _summary_cantonese
    _summary_cantonese = response.choices[0].message["content"]
    f = open("conversation.txt", "a", encoding="utf-8")
    summary_string = "---Summary---" + "\n" + _summary_cantonese + "\n" + "-----------------------" + "\n"
    f.write(summary_string)
    f.close()
    return _summary_cantonese
...
with gr.Blocks() as demo:
    chatbot = gr.Chatbot(value=[["","你好,我是Kaia,一名Kossie教練AI助手。你今天感覺如何?"]])
    msg = gr.Textbox()
    ...
    summarize_cantonese = gr.Button("Get summary in Cantonese")
    summary_cantonese_box = gr.Textbox(label="Summary Cantonese")
    summarize_cantonese.click(fn=summary_cantonese, inputs=None, outputs=summary_cantonese_box, queue=False)
